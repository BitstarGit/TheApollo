           ...
           if (empty($login)) {
	        	$errors[] = "Incorrect username or password";
	        	  ...
	        	  $data       = array(
		            ...
		            'errors'         => array(
		                'error_id'   => '1',
		                'error_text' => 'Your account is not activated yet, please check your inbox for the activation link'
		            )
		        );
              }
	            ...
	            if ($send_message) {
	            	$data = array(
	            	 ...
	            	 $data       = array(
			            ...
			            'errors'         => array(
			                'error_id'   => '0',
			                'error_text' => 'Error found while sending the reset link, please try again later'
			            )
			        );
	...
	if (!empty($_POST)) {
	    if (empty($_POST['password']) || empty($_POST['c_password']) || empty($_POST['email_code'])) {
	        $data    = array(
		        ...
		        'errors' => array(
		            'error_id' => '1',
		            'error_text' => 'Invalid request  or Missing arguments'
		        )
		    );
	    } else {
