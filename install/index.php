...
$config_file = file_put_contents($config_file_name, $file_content);
    if ($config_file) {
        ...
        if ($query) {
            ...
            $query_one  = mysqli_query($con2, "UPDATE `config` SET `value` = '" . mysqli_real_escape_string($con, $_POST['siteName']). "' WHERE `name` = 'name'");
            $query_one .= mysqli_query($con2, "UPDATE `config` SET `value` = '" . mysqli_real_escape_string($con, $_POST['siteTitle']). "' WHERE `name` = 'title'");
            $query_one .= mysqli_query($con2, "UPDATE `config` SET `value` = '" . mysqli_real_escape_string($con, $_POST['siteEmail']). "' WHERE `name` = 'email'");
            $query_one .= mysqli_query($con2, "INSERT INTO `users` (`username`,`password`, `email`, `admin`, `active`, `verified`, `last_active`, `registered`, `startup`) VALUES ('" . mysqli_real_escape_string($con2, $_POST['admin_username']). "', '" . mysqli_real_escape_string($con, sha1($_POST['admin_password']) ) . "','" . mysqli_real_escape_string($con, $_POST['siteEmail']) . "','1', '1', '1', '".time()."','".date('Y') . '/' . intval(date('m'))."', '2');");
